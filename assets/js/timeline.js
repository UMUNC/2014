    ...
    this._loadFacebook = function(callback) {
        ...
        var _done = function() {
            ...
            if (SELF._options.onSearchSuccess) {
                SELF._options.onSearchSuccess(data);
            }
        };

        var _initContent = function(facebook_data) {
            var content = '<div class="facebook_type_' + facebook_data.type + '">';
            content += '<div class="facebook_left_column"><img class="facebook_profile" src="https://graph.facebook.com/' + facebook_data.from.id + '/picture?type=square" /></div>';
            ...
            $(data.data).each(function(index, facebook_data) {
                if (facebook_data.from.id) {
                    if (facebook_data.type === 'photo') {
                        picture_in_loading++;
                        FB.api('/' + facebook_data.object_id, api_option, function(data) {
                            var content = _initContent(facebook_data);

                            if (data.source) {
                                content += '<div class="facebook_post">' +
                                               '<a href="' + facebook_data.link + '" style="display:inline;"><img class="facebook_picture" align="left" src="' + data.source +  '" /></a>' +
                                           '</div>';
                            }
